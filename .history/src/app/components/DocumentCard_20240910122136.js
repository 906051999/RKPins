...
const DocumentCard = ({ document, onClick, highlightedText }) => {
  ...
  useEffect(() => {
    ...
    const contentExcerpt = document.content ? document.content.slice(0, 100).trim() + '...' : '';
    setExcerpt(contentExcerpt);
    
    // 检查是否为迷你卡片：内容为空且没有标签
    const shouldBeMini = !document.content && (!document.tags || document.tags.length === 0);
    setIsMini(shouldBeMini);
    console.log('Is mini:', shouldBeMini); // 添加日志
  ...
  if (isMini) {
    console.log('Rendering mini card'); // 添加日志
    return (
      <div 
        className="bg-white bg-opacity-80 rounded-lg shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow duration-300 cursor-pointer flex items-center justify-center p-3 w-40 h-24"
        onClick={onClick}
      >
        <h2 className="font-bold text-sm text-gray-800 text-center break-words">
  ...
  console.log('Rendering full card'); // 添加日志
