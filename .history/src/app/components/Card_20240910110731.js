...
export default function Card({ website, onClick, highlightedText }) {
  ...
  const logoUrlRef = useRef(website.logo);
  ...
  const handleImageError = () => {
    clearTimeout(timerRef.current);
    console.error('加载图片失败:', logoUrlRef.current);
    setImageError(true);
  };

  useEffect(() => {
    if (logoUrlRef.current !== website.logo) {
      setImageLoaded(false);
      setImageError(false);
      logoUrlRef.current = website.logo;
